...
type AssetFilter = "all" | "authorized" | "needs_review" | "unauthorized" | "agent" | "no_agent" | "online" | "offline" | "unknown_status" | "windows" | "linux" | "android" | "ios" | "mobile" | "tablet" | "server" | "router" | "switch" | "access_point" | "printer" | "nas" | "camera" | "iot" | "unknown" | "ethernet" | "wifi" | "medium_unknown";
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-raven-border bg-raven-panel/85 p-4">
        ...
        <p className="mt-3 text-xs text-raven-muted">{t("Authorized CIDR")}: {safeArray(config?.allowed_cidrs).join(", ") || t("Not configured")} · {t("Discovery interval")}: {safeNumber(config?.discovery_interval_seconds, 300)}s · {t("Bounded TCP reachability")}: {config?.ping_enabled ? t("Enabled") : t("Disabled")} · {t("Service observations")}: {config?.service_check_enabled ? t("Enabled") : t("Disabled")}</p>
      </section>

      {!agentsOnly ? <section className="rounded-lg border border-raven-border bg-raven-panel/85 p-4"><div className="flex flex-wrap gap-2"><State label={t("Automatic asset registration")} active={Boolean(config?.auto_registration_enabled)} /><State label={t(config?.runtime_profile === "desktop" ? "Native host provider" : "ServerHost agent")} active={config?.runtime_profile === "desktop" ? config?.provider_status === "available" : Boolean(config?.server_host_agent_connected)} /><State label={t("Neighbor collector")} active={config?.neighbor_collector_status === "available"} /></div><div className="mt-3 grid gap-2 text-xs text-raven-muted sm:grid-cols-2 lg:grid-cols-4"><p>{t("Provider source")}: {t(config?.provider_source === "native_host_provider" ? "Native host provider" : safeString(config?.provider_source, "unavailable"))}</p><p>{t("Provider status")}: {t(safeString(config?.provider_status, "unavailable"))}</p><p>{t("Authorized CIDR")}: {safeArray(config?.allowed_cidrs).join(", ") || t("Not configured")}</p><p>{t("Neighbor collector status")}: {t(safeString(config?.neighbor_collector_status, "unavailable"))}</p><p>{t("Last automatic discovery")}: {safeDate(config?.last_discovery_at)?.toLocaleString() ?? t("Never")}</p><p>{t("Next automatic discovery")}: {safeDate(config?.next_discovery_at)?.toLocaleString() ?? t("Not scheduled")}</p><p>{t("Last service observation")}: {safeDate(config?.last_service_observation_at)?.toLocaleString() ?? t("Never")}</p><p>{t("Next service observation")}: {safeDate(config?.next_service_observation_at)?.toLocaleString() ?? t("Not scheduled")}</p><p>{t("Discovered assets")}: {safeNumber(config?.discovered_asset_count)}</p><p>{t("Raw observations")}: {safeNumber(config?.neighbor_raw_observations)}</p><p>{t("Accepted observations")}: {safeNumber(config?.neighbor_accepted_observations)}</p><p>{t("Rejected observations")}: {safeNumber(config?.neighbor_rejected_observations)}</p><p>{t("Deduplicated observations")}: {safeNumber(config?.neighbor_deduplicated_observations)}</p><p>{t("Out-of-CIDR observations")}: {safeNumber(config?.neighbor_out_of_cidr_observations)}</p><p>{t("Assets created")}: {safeNumber(config?.neighbor_assets_created)}</p><p>{t("Assets updated")}: {safeNumber(config?.neighbor_assets_updated)}</p></div>{config?.runtime_profile === "desktop" && config?.provider_status === "limited" ? <p className="mt-3 rounded border border-cyan-300/30 p-3 text-xs text-cyan-100">{t("Limited LAN visibility. RavenTech can observe the local network interface but discovered no peers. Network segmentation, client isolation, firewall policy, or inactive devices may limit visibility.")}</p> : null}{config?.runtime_profile !== "desktop" && !config?.neighbor_collector_active ? <p className="mt-3 text-xs text-cyan-100">{t("Start the manual ServerHost agent to populate read-only host neighbor observations. An empty sample is informational, not a platform failure.")}</p> : null}</section> : null}

      <section className="rounded-lg border border-raven-border bg-raven-panel/85 p-3"><p className="mb-2 text-xs uppercase tracking-wide text-raven-muted">{t("Asset filters")}</p><div className="flex flex-wrap gap-2">{(["all", "authorized", "needs_review", "unauthorized", "agent", "no_agent", "online", "offline", "unknown_status", "windows", "linux", "android", "ios", "mobile", "tablet", "server", "router", "switch", "access_point", "printer", "nas", "camera", "iot", "unknown", "ethernet", "wifi", "medium_unknown"] as AssetFilter[]).map((filter) => <button type="button" key={filter} onClick={() => setAssetFilter(filter)} className={`rounded-full border px-3 py-1 text-xs ${assetFilter === filter ? "border-raven-cyan bg-raven-cyan/10 text-raven-cyan" : "border-raven-border text-raven-muted"}`}>{t(filterLabel(filter))}</button>)}</div></section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <Metric label="Unknown status" value={modeAssets.filter((asset) => asset.status === "unknown").length} />
          ...
          <table className="w-full min-w-[1400px] text-left text-sm">
            ...
            <tbody>{assets.map((asset) => (
              <tr key={asset.id} className="border-t border-raven-border align-top hover:bg-raven-panelSoft/60">
                ...
                <td className="p-3">{t(asset.connection_medium)}<br /><span className="text-xs text-raven-muted">{asset.connection_medium === "unknown" ? t("Physical medium is not inferred without reliable evidence.") : `${safeString(asset.connection_medium_source, t("Evidence source"))} · ${t(asset.connection_medium_confidence)}`}</span></td>
...
function assetMatchesFilter(asset: LanAsset, filter: AssetFilter): boolean {
  ...
  if (["mobile", "tablet", "server", "router", "switch", "access_point", "printer", "nas", "camera", "iot"].includes(filter)) return asset.device_type === filter;
  if (filter === "unknown") return asset.os_family === "unknown" || asset.device_type === "unknown";
  if (filter === "unknown_status") return asset.status === "unknown";
  if (filter === "medium_unknown") return asset.connection_medium === "unknown";
  if (filter === "ethernet" || filter === "wifi") return asset.connection_medium === filter;
...
function filterLabel(filter: AssetFilter): string {
  const labels: Record<AssetFilter, string> = {
    ...
    windows: "Windows", linux: "Linux", android: "Android", ios: "iOS", mobile: "Mobile", tablet: "Tablet", server: "Server", router: "Router", switch: "Switch", access_point: "Access point", printer: "Printer", nas: "NAS", camera: "Camera", iot: "IoT", unknown: "Unknown",
    ethernet: "Ethernet", wifi: "Wi-Fi", medium_unknown: "Medium unknown",
  };
