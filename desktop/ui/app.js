...
const translations = {
  en: {
    ...
    pathSaved: "Project path validated and saved.", pathRejected: "That path is not a complete RavenTech repository. Nothing was saved.", pathCleared: "Saved path cleared. Automatic discovery remains available.",
    ...
    startBackend: "Start the approved local Docker services.", startFrontend: "Start the local Vite frontend.", fixMigrations: "Use the approved platform start/check flow to resolve pending migrations.", releaseMismatchAction: "The backend release does not match 5.0.0-rc4.", runtimeReady: "Local runtime is ready.",
    ...
    start: "Start platform", stop: "Stop platform", restart: "Restart platform", check: "Check health", openFrontend: "Open frontend", frontendCommand: "Run frontend dev", backendCommand: "Run Docker services", copy: "Copy", run: "Run", copied: "Command copied. Nothing was executed.",
  ...
  es: {
    ...
    pathSaved: "Ruta del proyecto validada y guardada.", pathRejected: "La ruta no es un repositorio completo de RavenTech. No se guardó nada.", pathCleared: "Ruta guardada borrada. El descubrimiento automático sigue disponible.",
    ...
    startBackend: "Inicia los servicios Docker locales aprobados.", startFrontend: "Inicia el frontend Vite local.", fixMigrations: "Usa el inicio/comprobación aprobado para resolver migraciones pendientes.", releaseMismatchAction: "La versión del backend no coincide con 5.0.0-rc4.", runtimeReady: "El entorno local está listo.",
    ...
    endpoints: "Endpoints locales", frontendUrl: "URL del frontend", backendUrl: "URL del backend", connected: "Frontend local conectado", showStatus: "Estado de servicios", openWorkspace: "Abrir espacio local", checking: "Comprobando…", reachable: "Accesible", unreachable: "No accesible", ready: "Listo", degraded: "Degradado", available: "Disponible", unavailable: "No disponible", unknown: "Desconocido",
    start: "Iniciar plataforma", stop: "Detener plataforma", restart: "Reiniciar plataforma", check: "Comprobar salud", openFrontend: "Abrir frontend", frontendCommand: "Ejecutar frontend dev", backendCommand: "Ejecutar servicios Docker", copy: "Copiar", run: "Ejecutar", copied: "Comando copiado. No se ejecutó nada.",
...
function renderCommands() {
  document.querySelector("#commands").replaceChildren(...commands.map((command) => {
    ...
    copyButton.addEventListener("click", async () => { try { await navigator.clipboard.writeText(command.text); document.querySelector("#message").textContent = copy("copied"); } catch { document.querySelector("#message").textContent = command.text; } });
...
async function saveProjectPath(event) {
  ...
  try { const result = await window.__TAURI__.core.invoke("bind_project_path", { projectPath: input.value }); message.textContent = copy(result.success ? "pathSaved" : "pathRejected"); message.className = result.success ? "ok" : "offline"; renderSetup(result.setup); }
...
function renderSnapshot(snapshot) {
  renderSetup(snapshot.setup); renderChecklist(snapshot); document.querySelector("#next-action").textContent = copy(nextRuntimeAction(snapshot));
