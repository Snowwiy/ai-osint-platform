"""Phase 3H audit hardening.

Revision ID: 0008_phase3h_audit_hardening
Revises: 0007_phase3g_case_management
...
revision: str = "0008_phase3h_audit_hardening"
down_revision: Union[str, None] = "0007_phase3g_case_management"
...
def upgrade() -> None:
    ...
    op.execute("UPDATE audit_logs SET created_at = timestamp")
...
def downgrade() -> None:
    if _index_exists("audit_logs", "idx_audit_logs_resource_type"):
        op.drop_index("idx_audit_logs_resource_type", "audit_logs")
    if _index_exists("audit_logs", "idx_audit_logs_investigation_id"):
        op.drop_index("idx_audit_logs_investigation_id", "audit_logs")
    if _index_exists("audit_logs", "idx_audit_logs_actor_id"):
        op.drop_index("idx_audit_logs_actor_id", "audit_logs")
    if _index_exists("audit_logs", "idx_audit_logs_created_at"):
    ...
    if _foreign_key_exists("audit_logs", "fk_audit_logs_actor_id_users"):
        op.drop_constraint(
            "fk_audit_logs_actor_id_users",
            "audit_logs",
            type_="foreignkey",
        )
    if _column_exists("audit_logs", "created_at"):
        op.drop_column("audit_logs", "created_at")
    if _column_exists("audit_logs", "metadata"):
        op.drop_column("audit_logs", "metadata")
    if _column_exists("audit_logs", "investigation_id"):
        op.drop_column("audit_logs", "investigation_id")
    if _column_exists("audit_logs", "actor_id"):
