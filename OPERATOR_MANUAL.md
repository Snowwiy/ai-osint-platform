...
For native Windows/Linux backend and worker packaging, fixed CLI checks, external configuration, loopback binding, and desktop lifecycle supervision, see [NATIVE_BACKEND_PACKAGING.md](NATIVE_BACKEND_PACKAGING.md) and [NATIVE_RUNTIME_SUPERVISOR.md](NATIVE_RUNTIME_SUPERVISOR.md). Tauri starts and supervises its fixed backend/worker children; PostgreSQL remains external. Redis/Celery are optional in desktop mode; Docker compatibility remains.
